...
use Addframe\Mediawiki\Category;
...
//Todo these can be set via params or pointed at a config file..?
$option['site'] = 'enwiki';
$option['category'] = 'Living_people‎';
$option['titleRegex'] = null;
$option['wdConditions'] = array('p107' => 'q215627');
$option['onlyIfNotSet'] = true;
$option['property'] = 'p107';
$option['value'] = 'q215627';
...
$site = $wm->getSiteFromSiteid( $option['site'] );
if( !$site instanceof Site ){
	die("No such wiki");
}

$category = $site->newCategoryFromTitle( 'Category:'.$option['category'] );
if( !$category instanceof Category ){
	die("No such category");
}

/** @var $page Page */
foreach( $category->getCategoryMembers( 50, false ) as $member ){

	//Only use main namespace
	if( $member['ns'] != 0 ){
		continue;
	}

	$page = $site->newPageFromTitle( $member['title'] );

	//Check against page regex
	if( $option['titleRegex'] != null ){
		if( !preg_match($option['titleRegex'], $page->getTitle() ) ){
			continue;
		}
	}
	...
	//Check if already has claim
	if( $option['onlyIfNotSet'] == true ){
		if( count($entity->getClaims( $option['property'] ) ) > 0 ){
			continue;
		}
	}

	//check if has claim condition
	foreach( $option['wdConditions'] as $property => $value ){
		$claims = $entity->getClaims( $property );
		foreach ( $claims as $claim ) {
			print_r( $claim );
			//todo look at what the format of this is... This comparison is probably wrong...
			if( $claim != $value ){
				continue 3;
			}
		}
	...
	//set the stuff
//	$result = $entity->createClaim( 'value', $option['property'], json_encode( $ourCoord ) );
//	echo $entity->getId();
