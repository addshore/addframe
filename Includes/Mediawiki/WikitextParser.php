...
class WikitextParser {
	...
	public function doNoWikiAndComments() {
		...
		foreach ( $nowiki as $element ) {
			$merged[$element['start']] = array( 'type' => 'nowiki', 'code' => $element['code'], 'end' => $element['end'] );
		}
		foreach ( $comments as $element ) {
			$merged[$element['start']] = array( 'type' => 'comment', 'code' => $element['code'], 'end' => $element['end'] );
		}
		ksort( $merged );
		$approved = array();
	...
	public function nowiki() {
		...
		foreach ( $search as $found ) {
			$tags[] = array( 'start' => $found['start'], 'code' => $found['code'], 'length' => strlen( $found['code'] ), 'end' => $found['end'] );
		}
		return $tags;
	}
	...
	public function wikObject_htmlComments() {
		...
		foreach ( $search as $found ) {
			$comments[] = array( 'start' => $found['start'], 'code' => $found['code'], 'length' => strlen( $found['code'] ), 'end' => $found['end'] );
		}
		return $comments;
	}
				...
				if ( $wikObject_link['level'] > 0 ) {
					if ( $wikObject_link['level'] == 1 && $char == '|' ) {
						...
						if ( $char == ':' ) {
							...
							} elseif ( ! isset( $wikObject_link['namespace'] ) && in_array( ucfirst( strtolower( $wikObject_link['page'] ) ), $this->namespaces ) ) {
								$wikObject_link['namespace'] = ucfirst( strtolower( $wikObject_link['page'] ) );
								$wikObject_link['page'] = '';
							} elseif ( ! isset( $wikObject_link['wikObject_interwiki'] ) && in_array( strtolower( $wikObject_link['page'] ), $this->wikObject_interwiki ) ) {
