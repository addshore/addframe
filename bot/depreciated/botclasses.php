...
class wikipedia {
    ...
    */
    ...
    function lastedit ($user) {
        $x = $this->query('?action=query&list=usercontribs&uclimit=1&format=php&ucuser='.urlencode($user));
        return $x['query']['usercontribs'][0]['title'];
    }
	...
	function last2edits ($page) {
		$x = $this->query('?action=query&prop=revisions&format=php&titles='.urlencode($page).'&rvlimit=2');
		foreach($x['query']['pages'] as $page)
		{
			return Array($page['revisions'][0]['revid'],$page['revisions'][1]['revid']);
		}
	}
